...
const zakatMaalSpecific = ['ZAKAT MAAL', 'ZAKAT MAL', 'ZAKAT HARTA', 'ZAKAT PENGHASILAN', 'ZAKAT PROFESI', 'ZKT MAL', 'ZKT PENGHASILAN', 'Z MAAL', 'ZKT MAL', 'Z MAAL', 'PEMBERSIHAN HARTA'];
for (const k of ['ZAKAT MAAL', 'ZAKAT MAL', 'ZAKAT HARTA', 'ZAKAT PENGHASILAN', 'ZAKAT PROFESI', 'ZKT MAL', 'ZKT PENGHASILAN', 'Z MAAL', 'ZKT MAL', 'Z MAAL', 'PEMBERSIHAN HARTA']) {
  if (!prog11.keywords.some(e => e.toUpperCase() === k.toUpperCase())) prog11.keywords.push(k);
}
console.log('Fixed Zakat Maal (11) keywords:', prog11.keywords.length);

// ===== 2. Fix Zakat Parent (16) - fallback only, NO keyword matching =====
const prog16 = config.programs.find(p => p.id === '16');
prog16.keywords = prog16.keywords.filter(k => !['ZAKAT', 'ZAKAT MAL', 'ZAKAT MAAL', 'ZAKAT HARTA', 'ZAKAT PENGHASILAN', 'ZAKAT PROFESI', 'ZAKAT FITRAH', 'ZAKAT FITRI'].includes(k.toUpperCase()));
if (!prog16.keywords.includes('PEMBERSIHAN HARTA')) prog16.keywords.push('PEMBERSIHAN HARTA');
...
for (const v of fitrahVariants) {
  if (!prog9.keywords.some(e => e.toUpperCase() === v.toUpperCase())) prog9.keywords.push(v);
}
...
// ===== Youva donor -> Umum =====
const youvaDom = config.donors.find(d => d.name && d.name.toLowerCase().includes('youva'));
if (youvaDom) { youvaDom.defaultProgramId = ''; youvaDom.defaultCoa = 40201001; console.log('Youva -> Umum'); }

// ===== Maria Alesha -> Sarana Fisik =====
const mariaDom = config.donors.find(d => d.name && d.name.toLowerCase().includes('maria alesha'));
if (mariaDom) { mariaDom.defaultProgramId = '14'; mariaDom.defaultCoa = 40202502; console.log('Maria Alesha -> Sarana Fisik'); }

// ===== Leon -> Palestine =====
const leonDom = config.donors.find(d => d.name && d.name.toLowerCase().includes('leon'));
if (leonDom) { leonDom.defaultProgramId = '3'; leonDom.defaultCoa = 40202101; console.log('Leon -> Palestine'); }
