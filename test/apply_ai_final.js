...
config.programs.find(p => p.id === '12').keywords = config.programs.find(p => p.id === '12').keywords.filter(k => !['WAKAF QURAN'].some(x => x.toUpperCase() === k.toUpperCase()));
...
config.programs.find(p => p.id === '9').keywords.push(...fitrahVariants.filter(v => !config.programs.find(p => p.id === '9').keywords.some(e => e.toUpperCase() === v.toUpperCase())));
console.log('Added FITRAH variants to program 9');

// ===== QURBAN variants program 10 =====
config.programs.find(p => p.id === '10').keywords.push(...['KURBAN', 'QURBN', 'KRBN', 'QURBAN', 'QURBANI', 'AQIQAH'].filter(v => !config.programs.find(p => p.id === '10').keywords.some(e => e.toUpperCase() === v.toUpperCase())));
